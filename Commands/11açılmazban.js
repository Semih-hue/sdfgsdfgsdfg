...
module.exports.execute = async (client, message, args) => {
   ...
   return message.channel.send("**Bu komutu sadece <@801053851728740403> kullanabilir.**");
    ...
    if (banLimit >= 2) {
        message.guild.owner.send(embed.setDescription(`${message.author} (\`${message.author.id}\`) **adlı yetkili 30 dk içerisinde 2 tane ban attığı için ban yetkisi alındı.**`)).catch();
