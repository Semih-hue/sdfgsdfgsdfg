...
module.exports.execute = async (client, message, args) => {
    ...
    if (!message.member.roles.cache.has(ayar.jailHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`**Bu komutu kullanmak için gerekli izine sahip değilsin.**`)).then(x => x.delete({timeout: 10000}));
