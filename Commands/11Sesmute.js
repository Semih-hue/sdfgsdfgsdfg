...
module.exports.execute = async (client, message, args) => {
  let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter(ayar.conf).setColor(`RANDOM`).setTimestamp();
    if (!message.member.roles.cache.has(ayar.muteHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`**Bu komutu kullanmak için gerekli izinlere sahip değilsin.**`)).then(x => x.delete({timeout: 10000}));
  ...
  if (message.member.roles.highest.position <= uye.roles.highest.position) return message.channel.send(embed.setDescription(`**Belirttiğin kişi senden üst yetkide veya onunla aynı yetkidesin!**`)).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.vmuteLogKanali && client.channels.cache.has(ayar.vmuteLogKanali)) client.channels.cache.get(ayar.vmuteLogKanali).send(embed.setDescription(`**${uye} üyesi, ${message.author} tarafından** \`${yaziSure}\` **boyunca** \`${reason}\` **nedeniyle ses kanallarından mutelendi!**`)).catch();
