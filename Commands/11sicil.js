...
module.exports.execute = async (client, message, args) => {
    ...
    let sicil = data.length > 0 ? data.map((value, index) => `\`${index+1}.\` [**${value.komut}**] ${client.users.cache.get(value.mod) || value.mod} **tarafından** \`${value.sebep}\` **sebebiyle** \`${new Date(value.zaman).toTurkishFormatDate()}\` tarihinde cezalandırılmış.`).join("\n") : "**Bu Üyenin Ceza Bilgisi Bulunamadı!**"
