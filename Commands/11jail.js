...
module.exports.execute = async (client, message, args) => {
    ...
    if (!message.member.roles.cache.has(ayar.jailHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`**Bu komutu kullanmak için gerekli izinlere sahip değilsin.**`)).then(x => x.delete({timeout: 10000}));
    ...
    if (message.guild.channels.cache.has(ayar.jailLogKanali)) message.guild.channels.cache.get(ayar.jailLogKanali).send(embed.setDescription(`**${victim}, üyesine <@&833326887588724766> rolü ${message.author} tarafından verildi.(**\`#${cezaID}\`**)** \n**Sebep :** \`${reason}\``))
    if (ayar.cezapuanlog && client.channels.cache.has(ayar.cezapuanlog)) client.channels.cache.get(ayar.cezapuanlog).send(`**${victim} : aldığınız** \`#${cezaID}\` **ID'li ceza ile** \`${cpuan || '0'}\` **ceza puanına ulaştınız.**`).catch();
