...
module.exports.execute = async (client, message, args) => {
    ...
    if (!message.member.roles.cache.has(ayar.banHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`**Bu komutu kullanmak için gerekli izinlere sahip değilsin.**`)).then(x => x.delete({timeout: 10000}));
    ...
    if (!victim) return message.channel.send(embed.setDescription(`**Geçerli bir kullanıcı ID'si belirtmelisin.**`)).then(x => x.delete({timeout: 10000}));
    ...
    if (!victim || reason.length < 1) return message.channel.send(embed.setDescription(`**Komutu doğru kullanmalısın!**\n**Örnek :** \`${ayar.prefix || ""}ban @üye [sebep]\``)).then(x => x.delete({timeout: 10000}));
    ...
    if (bandb >= 2) {
        if (message.guild.channels.cache.has(ayar.yetkiasma)) message.guild.channels.cache.get(ayar.yetkiasma).send(embed.setDescription(`**${message.author} (\`${message.author.id}\`) **adlı yetkili 30 dk içerisinde 2 tane ban attığı için ban yetkisi alındı.**`)).catch();
